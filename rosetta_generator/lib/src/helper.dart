...
Method generateLoader(Stone stone) {
  ...
  var assetLoaderTemplate = "${stoneAssetsPath(stone)}/$strReplaceKey.json";
  ...
  return Method(
    (mb) => mb
      ...
      ..body = Block.of([
        ...
        assetLoader
            .call([
              literalString(assetLoaderTemplate)
                  .property('replaceAll')
                  .call([literalString(strReplaceKey), refAssetName])
            ])
            .awaited
...
List<Code> generateAssetName() {
  var asset = refLocale.property("languageCode");
  return <Code>[
    asset.assignVar(strAssetName).statement,
    const Code("if ("),
    const Code("isNotEmpty($strLocaleName.scriptCode)"),
    const Code(")"),
    const Code("$strAssetName += '_\${$strLocaleName.scriptCode}';"),
    const Code("if ("),
    const Code("isNotEmpty($strLocaleName.countryCode)"),
    const Code(")"),
    const Code("$strAssetName += '_\${$strLocaleName.countryCode}';"),
  ];
}
